    ...
    if (!document.getElementById('haute-vars')) {
      ...
      style.textContent = `
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        h1 { font-family: var(--font-display); font-size: clamp(1.6rem, 3vw, 2.2rem); font-weight: 600; color: var(--charcoal); line-height: 1.15; }
        h3 { font-family: var(--font-display); font-size: clamp(1.2rem, 2vw, 1.5rem); font-weight: 600; color: var(--charcoal); line-height: 1.2; }
        .divider { height: 1px; background: var(--border); margin-top: 1rem; position: relative; }
        .divider::after { content: ''; position: absolute; left: 0; top: 0; width: 40px; height: 2px; background: linear-gradient(90deg, var(--gold), var(--gold-light)); border-radius: 2px; }
        .section-label { font-family: var(--font-body); font-size: 0.68rem; font-weight: 700; letter-spacing: 0.16em; text-transform: uppercase; color: var(--gold); margin-bottom: 0.3rem; }
        .btn-primary { display: inline-flex; align-items: center; gap: 0.5rem; background: var(--forest); color: var(--white); padding: 0.85rem 2rem; font-family: var(--font-body); font-size: 0.82rem; font-weight: 600; letter-spacing: 0.1em; text-transform: uppercase; border: none; border-radius: var(--radius); cursor: pointer; transition: all 0.3s; }
        .btn-primary:hover:not(:disabled) { background: var(--forest-mid); transform: translateY(-2px); box-shadow: 0 4px 20px rgba(26,74,58,0.25); }
        .btn-primary:disabled { opacity: 0.6; cursor: not-allowed; }
        .btn-dark { display: inline-flex; align-items: center; gap: 0.5rem; background: transparent; color: var(--charcoal); padding: 0.85rem 1.6rem; font-family: var(--font-body); font-size: 0.82rem; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; border: 1.5px solid rgba(0,0,0,0.15); border-radius: var(--radius); cursor: pointer; transition: all 0.25s; }
        .btn-dark:hover:not(:disabled) { border-color: var(--charcoal); background: rgba(0,0,0,0.04); }
...
const initialPayment = {
  bookingAmount: '', bookingMode: '', bookingRefId: '',
  bookingImage: null, bookingRemark: '', payments: [],
};

const initialPersonal = {
  firstName: '', lastName: '', fatherHusbandName: '',
  streetAddress: '', city: '', mobileNo: '', pin: '',
  dob: '', age: '', gender: '', panNo: '', email: '', profession: '',
};

const initialDocs = {
  aadharCard: null, panCard: null,
  optionalDocName: '', optionalDoc: null,
  // Client ID fields
...
function RegistrationContent() {
  ...
  const initialProject = {
    projectName:    plotId ? projectNameFromUrl : '',
    city:           plotId ? cityFromUrl        : '',
    plotNo,
  ...
  const [generatedClientId, setGeneratedClientId] = useState('');   // NEW — returned by API
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const serializedPayments = await Promise.all(
        (paymentData.payments || []).map(async (inst) => {
          const instFile = inst.image?.file instanceof File
            ? inst.image.file
            : inst.image instanceof File ? inst.image : null;
          return {
            date:        inst.date   || '',
            mode:        inst.mode   || '',
            amount:      inst.amount || '',
      ...
      const payload = {
        plotId,
        // Client ID mode — tells the API whether to generate or use existing
        clientIdMode:     docData.isExistingClient ? 'existing' : 'new',
        existingClientId: docData.existingClientId || '',
        project:  projectData,
        payment: {
      ...
      // Store the client ID returned by the API so SuccessScreen can show it
      setGeneratedClientId(result.clientId || '');
      setSubmitted(true);
      scrollTop();
    } catch (err) {
        ...
        <div style={{ maxWidth: 780, margin: '0 auto', padding: '0 1.5rem' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
            <div style={{
              ...
              padding: '0.4rem 1.2rem', borderRadius: 'var(--radius)', marginBottom: '1rem',
            }}>
            ...
            {plotNo && (
              <div style={{
                marginTop: '0.75rem',
                display: 'inline-flex', alignItems: 'center', gap: '0.5rem',
                background: 'rgba(26,74,58,0.07)', border: '1px solid rgba(26,74,58,0.2)',
          ...
          <div style={{
            ...
            padding: '2.5rem 3rem', border: '1px solid var(--border)', boxShadow: 'var(--shadow)',
          }}>
            ...
            {submitted ? (
              <SuccessScreen
                formData={{
                  ...
                  clientId: generatedClientId,   // pass the ID to success screen
                }}
                onReset={handleReset}
              />
            ) : step === 1 ? (
              <ProjectDetails data={projectData} onChange={setProjectData} onNext={handleStep1Next} />
            ) : step === 2 ? (
              <PaymentPlan
                data={paymentData} onChange={setPaymentData}
                onNext={handleStep2Next} onBack={() => { setStep(1); scrollTop(); }}
              />
            ) : step === 3 ? (
              <PersonalDetails
                data={personalData} onChange={setPersonalData}
                onNext={handleStep3Next} onBack={() => { setStep(2); scrollTop(); }}
              />
            ) : (
              <>
                <Documentation
