    ...
    >
      <div
        style={{
          ...
          width: "100%",
          maxWidth: 460,
          boxShadow: "0 24px 80px rgba(13,47,36,0.3)",
      ...
      {open && (
        <div
          style={{
            ...
            borderRadius: "var(--radius-lg)",
            boxShadow: "0 12px 40px rgba(26,74,58,0.15)",
            marginTop: 4,
            overflow: "hidden",
          }}
        >
...
function BulkEditModal({ plots, sector, onClose, onSaved }) {
  ...
  const toggleAll = () =>
    setSelected((prev) =>
      prev.length === plots.length ? [] : plots.map((p) => p._id),
    );
  ...
  const handleDelete = async () => {
    ...
    if (
      !confirm(
        `Permanently delete ${selected.length} plot(s)? This cannot be undone.`,
      )
    )
      return;
    setSaving(true);
    ...
    setSaving(false);
    setDone(true);
    onSaved(selected, null); // null signals deletion
  };
  ...
  const handleApply = async () => {
    ...
    if (changes.pricePerSqYard.trim())
        ...
        <div style={{ padding: 28, overflowY: "auto", flex: 1 }}>
          {done ? (
            <div style={{ textAlign: "center", padding: 48 }}>
              ...
              <div
                style={{ fontSize: 13, color: "var(--gray)", marginBottom: 28 }}
              >
                {selected.length} plot(s) updated successfully.
              </div>
              <button
                onClick={onClose}
                style={{
            ...
            <>
              <div style={{ marginBottom: 24 }}>
                ...
                >
                  ...
                  >
                    Step 1 — Select Plots ({selected.length} selected)
                  </div>
                  <button
                    onClick={toggleAll}
                    style={{
              ...
              >
                ...
                >
                  Step 2 — Set New Values (leave blank to keep existing)
                </div>
                <div
                  style={{
                    display: "grid",
                ...
                >
                  <div>
                    <label style={labelStyle}>Area (Sq.Yd)</label>
                    <input
                      type="number"
                      placeholder="e.g. 200"
                      value={changes.area}
                      onChange={(e) =>
                        setChanges((p) => ({ ...p, area: e.target.value }))
                      }
                      style={inputStyle}
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="e.g. 40'x45'"
                      value={changes.dimensions}
                      onChange={(e) =>
                        setChanges((p) => ({
                          ...p,
                  ...
                  <div>
                    ...
                    <select
                      value={changes.facing}
                      onChange={(e) =>
                        setChanges((p) => ({ ...p, facing: e.target.value }))
                      }
                      style={selectStyle}
                    >
                  ...
                  <div>
                    <label style={labelStyle}>Price / Sq.Yd (Rs)</label>
                    <input
                      type="number"
                      placeholder="e.g. 4500"
                      value={changes.pricePerSqYard}
                      onChange={(e) =>
                        setChanges((p) => ({
                          ...p,
                          pricePerSqYard: e.target.value,
                        }))
                      }
                      style={inputStyle}
                    />
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setChanges((p) => ({ ...p, status: e.target.value }))
                      }
                      style={selectStyle}
                    >
                      <option value="">— no change —</option>
                  ...
                  <div>
                    ...
                    <select
                      value={changes.corner}
                      onChange={(e) =>
                        setChanges((p) => ({ ...p, corner: e.target.value }))
                      }
                      style={selectStyle}
                    >
                      <option value="">— no change —</option>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
                <button
                  ...
                  style={{
                    ...
                    padding: "13px",
                    background:
                      selected.length > 0 ? "var(--gold)" : "var(--gray-light)",
                ...
                >
                  {saving
                    ? "Applying..."
                    : `Apply to ${selected.length} Plot${selected.length !== 1 ? "s" : ""}`}
                </button>
                <button
                  ...
                  style={{
                    ...
                    background:
                      selected.length > 0
                        ? "rgba(139,26,26,0.08)"
                        : "var(--gray-light)",
                    color: selected.length > 0 ? "#8b1a1a" : "var(--gray)",
                    border: `1px solid ${selected.length > 0 ? "rgba(139,26,26,0.3)" : "transparent"}`,
                    borderRadius: "var(--radius)",
                    fontSize: 13,
                    fontWeight: 700,
                    cursor: selected.length > 0 ? "pointer" : "not-allowed",
...
export default function LayoutPage() {
  ...
  const sectorPlots = selectedSector
    ? allPlots
        .filter((p) => p.sector === selectedSector)
        .sort((a, b) => {
          const toNum = (n) => (n === "12A" ? 12.5 : parseInt(n, 10));
          return toNum(a.plotNo) - toNum(b.plotNo);
        })
    : [];
  const uniqueSizes = [...new Set(sectorPlots.map((p) => p.area))].sort(
    (a, b) => b - a,
  );
  const filteredPlots = sectorPlots.filter((p) => {
    if (filterStatus !== "all" && p.status !== filterStatus) return false;
    if (filterSize !== "all" && String(p.area) !== filterSize) return false;
    if (search && !p.plotNo.toLowerCase().includes(search.toLowerCase()))
      return false;
    return true;
  });
  ...
  const handleAddSector = async () => {
    ...
    const res = await fetch("/api/sectors", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: newSectorName, projectId: project._id }),
    });
    const data = await res.json();
    setSaving(false);
    if (res.ok) {
      setSectors((prev) => [...prev, data.sector.name].sort());
      setNewSectorName("");
  ...
  const handleDeleteSector = async (sector) => {
    if (
      !confirm(
        `Delete sector "${sector}" and ALL its plots? This cannot be undone.`,
      )
    )
      return;
  ...
  const handleAddPlot = async () => {
    if (!newPlot.plotNo || !newPlot.area || !newPlot.dimensions) {
      alert("Plot No, Area and Dimensions are required");
      return;
    }
    setSaving(true);
    ...
    setSaving(false);
    if (res.ok) {
      setAllPlots((prev) => [...prev, data.plot]);
      setNewPlot({
        plotNo: "",
        area: "",
        dimensions: "",
  ...
  const handleDeletePlot = async (plot) => {
    if (!confirm(`Delete Plot No. ${plot.plotNo}? This cannot be undone.`))
      return;
  ...
  const handleEditPlot = async () => {
    if (!editPlot._id) {
      alert("Plot ID missing, please close and try again");
      return;
    }
    setSaving(true);
    const { _id, ...fields } = editPlot;
    const res = await fetch(`/api/plots/${_id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    if (res.ok) {
      setAllPlots((prev) =>
        prev.map((p) => (p._id === data.plot._id ? data.plot : p)),
      );
  ...
  const handleHoldSubmit = async () => {
    ...
    const res = await fetch(`/api/plots/${selectedPlot._id}`, {
      ...
      body: JSON.stringify({
        status: "hold",
        heldById: holdForm.employeeId,
        heldByName: holdForm.employeeName,
      }),
    });
    const data = await res.json();
    setSaving(false);
  ...
  const handleBulkSaved = (selectedIds, updates) => {
    if (updates === null) {
      setAllPlots((prev) => prev.filter((p) => !selectedIds.includes(p._id)));
    } else {
      setAllPlots((prev) =>
        prev.map((p) => (selectedIds.includes(p._id) ? { ...p, ...updates } : p)),
      );
    }
  };
  ...
  const handleReleaseHold = async (plot) => {
    ...
    const res = await fetch(`/api/plots/${plot._id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <>
      ...
      >
            ...
            >
              ...
              <div
                style={{
                  width: 1,
                  height: 28,
                  background: "rgba(201,144,26,0.3)",
                }}
              />
              <div>
                <div
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              ...
              {isAdmin && (
                <button
                  ...
                  style={{ ...btnOutlineForest, fontSize: 11 }}
                >
                  <svg
                    width="13"
                    height="13"
                ...
                >
                  {Object.entries(STATUS).map(([k, v]) => (
                    ...
                    >
                      ...
                      />
                      <span style={{ fontWeight: 600, color: v.color }}>
                        {v.label}
                      </span>
                    </div>
                  ))}
                  ...
                  <div key={a._id} style={{ position: "relative" }}>
                    <button
                      ...
                      style={{
                        ...
                        textAlign: "left",
                        background:
                          selectedAmenity?._id === a._id
                            ? "rgba(201,144,26,0.08)"
                            : "transparent",
                    ...
                    >
                      <span
                        style={{
                          ...
                          color: "var(--charcoal)",
                          fontWeight:
                            selectedAmenity?._id === a._id ? 600 : 400,
                        }}
                      >
                        {a.label}
                      </span>
            ...
            <div style={{ flex: 1, overflowY: "auto", height: "100%" }}>
              ...
              {!selectedSector && !selectedAmenity && (
                <div style={{ padding: 32 }}>
                  ...
                  >
                    Select a sector from the sidebar to browse plots and check
                    availability.
                  </p>
                  ...
                  >
                    ...
                    >
                      {[
                        ...
                        {
                          label: "Sold",
                          value: globalStats.sold,
                          color: "#8b1a1a",
                          bg: "rgba(139,26,26,0.06)",
                        },
                        {
                          label: "On Hold",
                          value: globalStats.hold,
                          color: "var(--gold)",
                          bg: "rgba(201,144,26,0.06)",
                        },
                      ].map((card) => (
                        <div
                          key={card.label}
                    ...
                    >
                      <img
                        src={
                          project?.layoutImage ||
                          (project?.name === "Haute World City"
                            ? "/assets/hwc-layout.jpg"
                            : project?.name === "Haute Grand City"
                              ? "/assets/hgc-layout.jpg"
                              : "/assets/expressway-layout.jpg")
                        }
                        alt="Layout Map"
                        style={{
                          width: "100%",
              ...
              {selectedSector && (
                <div style={{ padding: 28 }}>
                  ...
                  >
                    {Object.entries(STATUS).map(([k, v]) => {
                      const count = sectorPlots.filter(
                        (p) => p.status === k,
                      ).length;
                      return (
                        <div
                    ...
                    {filteredPlots.map((plot) => {
                      ...
                      return (
                        <button
                          key={plot._id}
                          onClick={() =>
                            setSelectedPlot(isSelected ? null : plot)
                          }
                          style={{
                            ...
                            boxShadow: isSelected
                              ? "0 4px 16px rgba(201,144,26,0.25)"
                              : "none",
                          }}
                          onMouseEnter={(e) => {
                            if (!isSelected)
                        ...
                        >
                          <div
                            style={{
                              fontSize: 13,
                              fontWeight: 700,
                              color: isSelected ? "var(--white)" : st.color,
                              fontFamily: "var(--font-body)",
                            }}
                          >
                            {plot.plotNo}
                    ...
                    >
                      <div
                        style={{
                          ...
                          height: 3,
                          background:
                            STATUS[selectedPlot.status]?.color || "var(--gold)",
                        }}
                      />

                      {(() => {
                        const currentIdx = filteredPlots.findIndex(
                          (p) => p._id === selectedPlot._id,
                        );
                        const prevPlot = filteredPlots[currentIdx - 1];
                        const nextPlot = filteredPlots[currentIdx + 1];
                        return (
                          <div
                            style={{
                              display: "flex",
                          ...
                          >
                            <button
                              onClick={() =>
                                prevPlot && setSelectedPlot(prevPlot)
                              }
                              disabled={!prevPlot}
                              style={{
                                display: "inline-flex",
                                ...
                                fontFamily: "var(--font-body)",
                                fontWeight: 700,
                                cursor: prevPlot ? "pointer" : "not-allowed",
                                letterSpacing: "0.06em",
                                textTransform: "uppercase",
                                opacity: prevPlot ? 1 : 0.4,
                              }}
                            >
                              <svg
                                width="13"
                              ...
                              {prevPlot ? `Plot ${prevPlot.plotNo}` : "No Prev"}
                            </button>
                            <span
                              style={{
                                fontSize: 11,
                            ...
                            >
                              {currentIdx + 1} / {filteredPlots.length}
                            </span>
                            <button
                              onClick={() =>
                                nextPlot && setSelectedPlot(nextPlot)
                              }
                              disabled={!nextPlot}
                              style={{
                                display: "inline-flex",
                                ...
                                fontFamily: "var(--font-body)",
                                fontWeight: 700,
                                cursor: nextPlot ? "pointer" : "not-allowed",
                                letterSpacing: "0.06em",
                                textTransform: "uppercase",
                                opacity: nextPlot ? 1 : 0.4,
                              }}
                            >
                              {nextPlot ? `Plot ${nextPlot.plotNo}` : "No Next"}
                              <svg
                                width="13"
                                height="13"
                                viewBox="0 0 24 24"
                      ...
                      >
                        {[
                          {
                            label: "Area",
                            value: `${selectedPlot.area} Sq.Yd`,
                          },
                          {
                            label: "Dimensions",
                            value: selectedPlot.dimensions,
                          },
                          {
                            label: "Facing",
                          ...
                          {
                            label: "Corner Plot",
                            value: selectedPlot.corner ? "Yes" : "No",
                          },
                          {
                            label: "Price/Sq.Yd",
                            value: selectedPlot.pricePerSqYard
                              ? `Rs ${selectedPlot.pricePerSqYard.toLocaleString("en-IN")}`
                              : "—",
                          },
                        ].map((item) => (
                          <div
                      ...
                      {selectedPlot.status === "hold" &&
                        selectedPlot.holdUntil && (
                          ...
                          >
                            ...
                            >
                              Held by:{" "}
                              <strong>{selectedPlot.heldByName}</strong>{" "}
                              <span style={{ color: "var(--gray)" }}>
                                (ID: {selectedPlot.heldById})
                              </span>
                            </div>
                            <div
                              style={{
                            ...
                            >
                              Time remaining:{" "}
                              <HoldTimer holdUntil={selectedPlot.holdUntil} />
                            </div>
                            {isAdmin && (
                              <button
                        ...
                        >
                          {selectedPlot.status === "available" && (
                            <>
                              <button
                                onClick={() =>
                                  (() => { localStorage.setItem('layoutUrl', window.location.pathname); router.push(`/registration?plotId=${selectedPlot._id}&plotNo=${selectedPlot.plotNo}&sector=${selectedPlot.sector}&area=${selectedPlot.area}&price=${selectedPlot.pricePerSqYard}&projectName=${encodeURIComponent(project?.name || "")}&city=${encodeURIComponent(project?.location || "")}`); })()
                                }
                                style={btnPrimary}
                              >
                                Book This Plot
                          ...
                          {selectedPlot.status === "hold" && isAdmin && (
                            <button
                              onClick={() =>
                                router.push(
                                  `/registration?plotId=${selectedPlot._id}&plotNo=${selectedPlot.plotNo}&sector=${selectedPlot.sector}&area=${selectedPlot.area}&price=${selectedPlot.pricePerSqYard}&projectName=${encodeURIComponent(project?.name || "")}&city=${encodeURIComponent(project?.location || "")}`,
                                )
                              }
                              style={btnPrimary}
                            >
        ...
        {showAddPlot && (
          <Modal
            title={`Add Plot to ${selectedSector}`}
            onClose={() => setShowAddPlot(false)}
          >
            <InputRow
              label="Plot No *"
              value={newPlot.plotNo}
              onChange={(v) => setNewPlot((p) => ({ ...p, plotNo: v }))}
              placeholder="e.g. 49"
            />
            <InputRow
              label="Area (Sq.Yd) *"
              value={newPlot.area}
              onChange={(v) => setNewPlot((p) => ({ ...p, area: v }))}
              placeholder="e.g. 200"
              type="number"
            />
            <InputRow
              ...
              onChange={(v) => setNewPlot((p) => ({ ...p, dimensions: v }))}
              placeholder="e.g. 40'x45'"
            />
            <InputRow
              label="Facing"
              value={newPlot.facing}
              onChange={(v) => setNewPlot((p) => ({ ...p, facing: v }))}
              placeholder="North / South / East / West"
            />
            <InputRow
              label="Price per Sq.Yd"
              value={newPlot.pricePerSqYard}
              onChange={(v) => setNewPlot((p) => ({ ...p, pricePerSqYard: v }))}
              placeholder="e.g. 4500"
              type="number"
            />
            <div
            ...
            >
              <input
                type="checkbox"
                checked={newPlot.corner}
                onChange={(e) =>
                  setNewPlot((p) => ({ ...p, corner: e.target.checked }))
                }
        ...
        {showEditPlot && (
          <Modal
            title={`Edit Plot No. ${editPlot.plotNo}`}
            onClose={() => setShowEditPlot(false)}
          >
            <InputRow
              label="Plot No"
              value={editPlot.plotNo}
              onChange={(v) => setEditPlot((p) => ({ ...p, plotNo: v }))}
              placeholder="e.g. 49"
            />
            <InputRow
              label="Area (Sq.Yd)"
              value={editPlot.area}
              onChange={(v) => setEditPlot((p) => ({ ...p, area: v }))}
              placeholder="e.g. 200"
              type="number"
            />
            <InputRow
              label="Dimensions"
              value={editPlot.dimensions}
              onChange={(v) => setEditPlot((p) => ({ ...p, dimensions: v }))}
              placeholder="e.g. 40'x45'"
            />
            <InputRow
              label="Facing"
              value={editPlot.facing}
              onChange={(v) => setEditPlot((p) => ({ ...p, facing: v }))}
              placeholder="North / South / East / West"
            />
            <InputRow
              label="Price per Sq.Yd"
              value={editPlot.pricePerSqYard}
              onChange={(v) =>
                setEditPlot((p) => ({ ...p, pricePerSqYard: v }))
              }
              placeholder="e.g. 4500"
              type="number"
            />
            <div style={{ marginBottom: 12 }}>
              ...
              <select
                ...
                onChange={(e) =>
                  setEditPlot((p) => ({ ...p, status: e.target.value }))
                }
                style={{
                  width: "100%",
                  padding: "10px 14px",
            ...
            >
              <input
                type="checkbox"
                checked={editPlot.corner || false}
                onChange={(e) =>
                  setEditPlot((p) => ({ ...p, corner: e.target.checked }))
                }
                style={{ accentColor: "var(--forest)", width: 16, height: 16 }}
              />
              <span
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setEditPlot((p) => ({ ...p, notes: e.target.value }))
                }
                rows={3}
                style={{
                  width: "100%",
        ...
        {showAddAmenity && (
          ...
          >
            <InputRow
              ...
              onChange={(v) => setNewAmenity((p) => ({ ...p, label: v }))}
              placeholder="e.g. Jogging Track"
            />
            <InputRow
              label="Area (optional)"
              value={newAmenity.area}
              onChange={(v) => setNewAmenity((p) => ({ ...p, area: v }))}
              placeholder="e.g. 1200 Sq.Yd"
            />
            <button
        ...
        {showLayoutMap && (
          ...
          >
            ...
            >
              ...
              >
                Layout Plan · Click outside to close
              </div>
            </div>
          </div>
        )}
