...
function getDriveAuthClient() {
  ...
  console.log('[OAuth] clientId present:', !!clientId);
  console.log('[OAuth] clientSecret present:', !!clientSecret);
  console.log('[OAuth] refreshToken present:', !!refreshToken);
...
// ── Atomically get next sequential number for a project ──────────────────────
async function getNextSeq(projectCode) {
...
export async function POST(request) {
  try {
    ...
    const safeName = [clientId, personal?.firstName, personal?.lastName, project?.plotNo]
      .filter(Boolean)
      .join('_')
    ...
    const instalmentSummary = instalments.length === 0
      ? ''
      : instalments.map((inst, i) =>
          `Inst ${i + 1}: Rs${inst.amount || 0} | ${inst.mode || '-'} | Ref: ${inst.refId || '-'} | Date: ${inst.date || '-'}${inst.remark ? ` | Note: ${inst.remark}` : ''}`,
        ).join('\n');
            ...
            <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 16px; background: #f9f6f2;">
              ...
              <div style="background: #fff; padding: 20px 16px; border-radius: 0 0 8px 8px; border: 1px solid #e8dfd4;">
                ...
                <p style="font-size: 14px; color: #4a3728; line-height: 1.8;">
                  We sincerely appreciate your decision to choose one of our upcoming landmark residential projects —
                  <strong>${project?.projectName || 'Expressway Residency'}</strong>.
                </p>

                <p style="font-size: 14px; color: #4a3728; line-height: 1.8;">
                  ${project?.projectName || 'Expressway Residency'} is an upcoming landmark project promoted by Haute Developers
                  bringing 15+ years of experience in delivering residential projects. The project entails a modern lifestyle,
                  serene living and premium experience, strategically located in ${project?.city || 'India'}.
                </p>

                <p style="font-size: 14px; color: #4a3728; line-height: 1.8;">Please find the booking details of your unit here.</p>

                <!-- Client ID highlight box -->
                <div style="background: #2c2418; border-radius: 8px; padding: 14px 20px; margin: 16px 0; text-align: center;">
                  <p style="font-size: 11px; color: #a08060; letter-spacing: 3px; text-transform: uppercase; margin: 0 0 6px;">Your Client ID</p>
                  <p style="font-size: 24px; font-weight: 700; color: #c9a87c; letter-spacing: 4px; margin: 0; font-family: monospace;">${clientId}</p>
                  <p style="font-size: 11px; color: #7a6a58; margin: 6px 0 0;">Please keep this ID for all future correspondence</p>
                </div>
                ...
                <div style="background: #f9f6f2; border: 1px solid #e8dfd4; border-radius: 8px; padding: 12px; margin: 12px 0 20px;">
                  <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
                    <tr>
                      <td style="padding: 8px 0; color: #9e8c7a; width: 40%;">Client ID</td>
                      <td style="padding: 8px 0; color: #2c2418; font-weight: 700; font-family: monospace; letter-spacing: 2px;">${clientId}</td>
                    </tr>
                    <tr style="border-top: 1px solid #e8dfd4;">
                      <td style="padding: 8px 0; color: #9e8c7a;">Plot No.</td>
                      <td style="padding: 8px 0; color: #2c2418; font-weight: 600;">${project?.projectName === 'Expressway Residency' ? project.sector + '-' : ''}${project?.plotNo}</td>
                    </tr>
                    <tr style="border-top: 1px solid #e8dfd4;">
                    ...
                    <tr style="border-top: 1px solid #e8dfd4;">
                      ...
                      <td style="padding: 8px 0; color: #2c2418; font-weight: 600;">Rs 1,00,000/-</td>
                    </tr>` : ''}
                    <tr style="border-top: 1px solid #e8dfd4;">
                      <td style="padding: 8px 0; color: #9e8c7a;">Total Cost</td>
                      <td style="padding: 8px 0; color: #2c2418; font-weight: 600;">Rs ${fmt(project?.totalCost)}/-</td>
                    </tr>
                  </table>
                </div>
                ...
                <p style="font-size: 13px; color: #7a6a58; line-height: 1.8; background: #fdf8f0; border-left: 3px solid #c9a87c; padding: 10px 14px; border-radius: 4px;">
                  ...
                  of payment, as discussed or as mutually agreed.
                </p>
                ...
                <div style="margin-top: 28px; padding-top: 20px; border-top: 1px solid #e8dfd4;">
                  ...
                  <p style="font-size: 12px; color: #9e8c7a; margin: 0 0 4px; word-break: break-word;">Corporate Address: H-214, Sector 63, Noida, Uttar Pradesh, India</p>
